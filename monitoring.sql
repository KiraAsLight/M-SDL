...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `CleanOldLogs` ()   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetDashboardSummary` ()   BEGIN
    ...
    SELECT 
        -- Today's statistics
        (SELECT COUNT(*) FROM log_aktivitas WHERE DATE(created_at) = today_date) as today_total,
        (SELECT COUNT(*) FROM log_aktivitas WHERE DATE(created_at) = today_date AND status = 'berhasil') as today_successful,
        (SELECT COUNT(*) FROM log_aktivitas WHERE DATE(created_at) = today_date AND status = 'gagal') as today_failed,
        (SELECT COUNT(DISTINCT user_name) FROM log_aktivitas WHERE DATE(created_at) = today_date) as today_unique_users,
        
        -- Card statistics
        (SELECT COUNT(*) FROM authorized_cards WHERE is_active = 1) as active_cards,
        (SELECT COUNT(*) FROM authorized_cards WHERE is_active = 0) as inactive_cards,
        
        -- Device status
        (SELECT COUNT(*) FROM device_status WHERE status = 'online') as online_devices,
        (SELECT COUNT(*) FROM device_status WHERE last_heartbeat > DATE_SUB(NOW(), INTERVAL 5 MINUTE)) as recent_devices;
END$$
...
--
-- Triggers `authorized_cards`
--
DELIMITER $$
CREATE TRIGGER `tr_authorized_cards_update` BEFORE UPDATE ON `authorized_cards` FOR EACH ROW BEGIN
...
(1, 'MAIN_DOOR', 'online', '{\"vibrationCount\":0,\"wifi_signal\":-23,\"uptime\":1281,\"door_locked\":true,\"system_armed\":false,\"free_memory\":32747}', '2025-08-01 03:08:52', '2025-07-31 15:12:13', '2025-08-01 03:08:52'),
(305, 'SYSTEM', 'online', '{}', '2025-08-01 02:59:58', '2025-08-01 02:59:58', '2025-08-01 02:59:58');

--
-- Triggers `device_status`
--
DELIMITER $$
CREATE TRIGGER `tr_device_status_update` BEFORE UPDATE ON `device_status` FOR EACH ROW BEGIN
    SET NEW.updated_at = CURRENT_TIMESTAMP;
END
$$
DELIMITER ;
...
(31, 'MAIN_DOOR', 'TEST_COMMAND', '{}', 'SENT', '2025-08-01 03:05:07', '2025-08-01 03:05:13');

--
-- Triggers `door_commands`
--
DELIMITER $$
CREATE TRIGGER `tr_door_commands_update` BEFORE UPDATE ON `door_commands` FOR EACH ROW BEGIN
    SET NEW.updated_at = CURRENT_TIMESTAMP;
END
$$
DELIMITER ;
...
CREATE TABLE `log_aktivitas` (
  ...
  `description` varchar(255) DEFAULT 'Door access',
...
INSERT INTO `log_aktivitas` (`id_log`, `card_id`, `user_name`, `status`, `description`, `device_id`, `created_at`) VALUES
...
(455, 'MIGRATION', 'Database Admin', 'berhasil', 'Database fields migrated to English naming convention', 'SYSTEM', '2025-08-01 01:57:18'),
(456, 'B34B2803', 'Anggota Reguler (S-1)', 'berhasil', 'Akses RFID berhasil', 'MAIN_DOOR', '2025-08-01 02:17:13'),
(457, 'CC412803', 'Unknown', 'gagal', 'Kartu tidak terdaftar', 'MAIN_DOOR', '2025-08-01 02:17:55'),
(458, '0581D6E14CA100', 'Admin System (E-KTP)', 'berhasil', 'Akses RFID berhasil', 'MAIN_DOOR', '2025-08-01 02:18:10'),
(459, 'B34B2803', 'Anggota Reguler (S-1)', 'berhasil', 'Akses RFID berhasil', 'MAIN_DOOR', '2025-08-01 02:20:55'),
(460, '0581D6E14CA100', 'Admin System (E-KTP)', 'berhasil', 'Akses RFID berhasil', 'MAIN_DOOR', '2025-08-01 02:21:03'),
(461, 'B34B2803', 'Anggota Reguler (S-1)', 'berhasil', 'Akses RFID berhasil', 'MAIN_DOOR', '2025-08-01 02:28:20'),
(462, '0581D6E14CA100', 'Admin System (E-KTP)', 'berhasil', 'Akses RFID berhasil', 'MAIN_DOOR', '2025-08-01 02:28:31'),
(463, 'B34B2803', 'Anggota Reguler (S-1)', 'berhasil', 'Akses RFID berhasil', 'MAIN_DOOR', '2025-08-01 02:47:39'),
(464, 'REMOTE_COMMAND', 'Remote Control', 'berhasil', 'Remote command executed: TEST_COMMAND', 'MAIN_DOOR', '2025-08-01 03:05:13'),
(465, '0581D6E14CA100', 'Admin System (E-KTP)', 'berhasil', 'Akses RFID berhasil', 'MAIN_DOOR', '2025-08-01 03:08:08'),
(466, '0581D6E14CA100', 'Admin System (E-KTP)', 'berhasil', 'Akses RFID berhasil', 'MAIN_DOOR', '2025-08-01 03:08:22');
...
`today_total` bigint(21)
,`today_successful` bigint(21)
,`today_failed` bigint(21)
...
,`sensors_data` text
,`total_commands` bigint(21)
,`pending_commands` bigint(21)
...
-- (See below for the actual view)
--
CREATE TABLE `v_recent_activities` (
`id_log` int(11)
,`card_id` varchar(50)
,`user_name` varchar(100)
,`status` enum('berhasil','gagal','pending')
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_dashboard_summary`  AS SELECT (select count(0) from `log_aktivitas` where cast(`log_aktivitas`.`created_at` as date) = curdate()) AS `today_total`, (select count(0) from `log_aktivitas` where cast(`log_aktivitas`.`created_at` as date) = curdate() and `log_aktivitas`.`status` = 'berhasil') AS `today_successful`, (select count(0) from `log_aktivitas` where cast(`log_aktivitas`.`created_at` as date) = curdate() and `log_aktivitas`.`status` = 'gagal') AS `today_failed`, (select count(0) from `authorized_cards` where `authorized_cards`.`is_active` = 1) AS `active_cards`, (select count(0) from `device_status`) AS `total_devices` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_device_summary`  AS SELECT `ds`.`device_id` AS `device_id`, `ds`.`status` AS `device_status`, `ds`.`last_heartbeat` AS `last_heartbeat`, `ds`.`sensors_data` AS `sensors_data`, count(`dc`.`id`) AS `total_commands`, count(case when `dc`.`status` = 'PENDING' then 1 end) AS `pending_commands`, count(`la`.`id_log`) AS `total_logs` FROM ((`device_status` `ds` left join `door_commands` `dc` on(`ds`.`device_id` = `dc`.`device_id`)) left join `log_aktivitas` `la` on(`ds`.`device_id` = `la`.`device_id`)) GROUP BY `ds`.`device_id`, `ds`.`status`, `ds`.`last_heartbeat`, `ds`.`sensors_data` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_recent_activities`  AS SELECT `la`.`id_log` AS `id_log`, `la`.`card_id` AS `card_id`, `la`.`user_name` AS `user_name`, `la`.`status` AS `status`, `la`.`description` AS `description`, `la`.`device_id` AS `device_id`, `la`.`created_at` AS `created_at`, `ac`.`user_name` AS `card_owner`, `ac`.`is_active` AS `card_active`, `ds`.`status` AS `device_status`, `ds`.`last_heartbeat` AS `last_heartbeat` FROM ((`log_aktivitas` `la` left join `authorized_cards` `ac` on(`la`.`card_id` = `ac`.`card_id`)) left join `device_status` `ds` on(`la`.`device_id` = `ds`.`device_id`)) ORDER BY `la`.`created_at` DESC ;
...
ALTER TABLE `authorized_cards`
  ...
  ADD UNIQUE KEY `uk_card_id` (`card_id`),
  ADD KEY `idx_authorized_cards_card_id` (`card_id`),
  ADD KEY `idx_authorized_cards_active` (`is_active`,`card_id`),
  ADD KEY `idx_authorized_cards_user` (`user_name`),
  ADD KEY `idx_authorized_cards_user_name` (`user_name`),
  ADD KEY `idx_cards_active` (`is_active`),
  ADD KEY `idx_cards_card_id` (`card_id`);
...
ALTER TABLE `device_status`
  ...
  ADD UNIQUE KEY `uk_device_status_device_id` (`device_id`),
  ADD UNIQUE KEY `uk_device_id` (`device_id`),
  ADD KEY `idx_device_status_device_id` (`device_id`),
  ADD KEY `idx_device_status_heartbeat` (`last_heartbeat`),
  ADD KEY `idx_device_status_active` (`device_id`,`last_heartbeat`),
  ADD KEY `idx_device_heartbeat` (`last_heartbeat`),
  ADD KEY `idx_device_device_id` (`device_id`);
...
ALTER TABLE `door_commands`
  ...
  ADD KEY `idx_door_commands_status` (`status`,`created_at`),
  ADD KEY `idx_door_commands_device_id` (`device_id`),
  ADD KEY `idx_door_commands_created_at` (`created_at`),
  ADD KEY `idx_commands_status` (`status`),
  ADD KEY `idx_commands_device_id` (`device_id`);
...
ALTER TABLE `log_aktivitas`
  ...
  ADD KEY `idx_log_aktivitas_card_id` (`card_id`),
  ADD KEY `idx_log_aktivitas_user_name` (`user_name`),
  ADD KEY `idx_log_aktivitas_status` (`status`),
  ADD KEY `idx_log_aktivitas_device_id` (`device_id`),
  ADD KEY `idx_card_id_lookup` (`card_id`),
  ADD KEY `idx_log_aktivitas_composite` (`created_at`,`status`),
  ADD KEY `idx_log_aktivitas_user_date` (`user_name`,`created_at`),
  ADD KEY `idx_log_aktivitas_device_date` (`device_id`,`created_at`),
  ADD KEY `idx_log_aktivitas_created_at` (`created_at`),
  ADD KEY `idx_log_user_name` (`user_name`),
  ADD KEY `idx_log_status` (`status`),
  ADD KEY `idx_log_created_at` (`created_at`),
  ADD KEY `idx_log_card_id` (`card_id`),
  ADD KEY `idx_log_device_id` (`device_id`);
...
--
-- Constraints for table `door_commands`
--
ALTER TABLE `door_commands`
  ADD CONSTRAINT `fk_door_commands_device` FOREIGN KEY (`device_id`) REFERENCES `device_status` (`device_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_door_commands_device_id` FOREIGN KEY (`device_id`) REFERENCES `device_status` (`device_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `log_aktivitas`
--
ALTER TABLE `log_aktivitas`
  ADD CONSTRAINT `fk_log_device_id` FOREIGN KEY (`device_id`) REFERENCES `device_status` (`device_id`) ON DELETE SET NULL ON UPDATE CASCADE;
COMMIT;
